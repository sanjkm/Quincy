...
# Looks at table back_month_billed to set those invoices to the
# correct BillingDate (one month before), InvoiceDate, and ServiceMonth
def back_month_billed_exceptions (month_num):

    billing_month = int(month_num) - 1
    if int(month_num) == 1:
        billing_month = 12
        
    billing_month = str(billing_month)

    billing_date = calc_full_date (billing_month)
    billing_date_str = (billing_date.__str__()).split(' ')[0]
    ...
    try:
        ...
        query = "UPDATE month_temp AS t1, back_month_billed AS t2 " \
                "SET t1.Invoice_Date = %s, " \
                "t1.Service_Month = %s, " \
                "t1.Billed_Month = %s "   \
                "WHERE t1.Attributable = t2.Attributable AND " \
        ...
        args = (invoice_date_str, service_date_str, billing_date_str)
        cursor.execute (query, args)
        conn.commit()
...
def main():
    ...
    back_month_billed_exceptions (month_num)
